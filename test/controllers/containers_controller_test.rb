...
class ContainersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create container" do
    assert_difference('Container.count') do
      post containers_url, params: { container: { name: @container.name, s3_uri: @container.s3_uri } }
    end
  ...
  test "should update container" do
    patch container_url(@container), params: { container: { name: @container.name, s3_uri: @container.s3_uri } }
